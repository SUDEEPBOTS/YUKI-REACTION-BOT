...
import config
from YUKIREACTION.mplugin.helpers import is_owner
from config import API_HASH, API_ID, OWNER_ID
from YUKIREACTION import CLONE_OWNERS
from YUKIREACTION import YUKIREACTION as app, save_clonebot_owner
from YUKIREACTION import db as mongodb, YUKIREACTION
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="YUKIREACTION/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
